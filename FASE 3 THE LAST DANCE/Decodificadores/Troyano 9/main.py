...
R_TYPE = {
    ...
    "MUL": {"opcode": "000000", "funct": "000010", "format": "RD, RS, RT"},
    ...
    "DIVU": {"opcode": "000000", "funct": "011011", "format": "RD, RS, RT"},
    "RSQRT": {"opcode": "000000", "funct": "010110", "format": "RD, RS, RT"},
    "NOP": {"opcode": "000000", "funct": "000000", "format": ""}
}

I_TYPE = {
    "ADDI": {"opcode": "001000", "format": "RT, RS, #inmediato"},
    "ORI": {"opcode": "001101", "format": "RT, RS, #inmediato"},
    "ANDI": {"opcode": "001100", "format": "RT, RS, #inmediato"},
...
def parse_instruction(line):
    ...
    # Tipo-J (Ej: J 0x00400000)
    elif mnemonic in J_TYPE:
        if len(parts) != 2:
            raise ValueError(f"Formato incorrecto. Uso: {mnemonic} #target")
        
        target = parts[1].replace('#', '').replace('0x', '')
        try:
            target_num = int(target, 16) if 'x' in target else int(target)
        except:
            raise ValueError(f"Target inválido: {target}")
        
        opcode = J_TYPE[mnemonic]["opcode"]
        target_bin = f"{target_num:026b}"  # 26 bits
        full_binary = f"{opcode}{target_bin}"
    ...
    def decode_file(self):
        ...
        try:
            for line in asm_content:
                ...
                line = line.split('//')[0].strip()  # Elimina todo después de //
                if not line:
                    continue
